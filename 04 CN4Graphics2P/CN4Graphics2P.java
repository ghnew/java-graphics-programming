...
public class CN4Graphics2P extends JFrame {
    ...
    public boolean hasWon(Seed theSeed, int rowSelected, int colSelected) {
        for (int col = 0, count = 0; col < COLS; ++col) {
        ...
        // Check diagonals
        for (int row = 0, dCount = 0, odCount = 0; row < ROWS; row++) {
            for (int col = 0; col < COLS; col++) {

                // TODO: check 4 in the diagonal

                // check 4 in the opposite diagonal
                if (col + row == rowSelected + colSelected) {
                    if (board[row][col] == theSeed) {
                        ++odCount;
                        if (odCount == 4) return true;
                    } else if (board[row][col] != theSeed){
                        odCount = 0;
                    }
                }
            }
